...
function postinst()
	...
	ret, out = cmdexec("mkdir -p /tmp/mountedmmcblk0p2")
	if (false == ret) then
		return ret, out
	end

	ret, out = cmdexec("mount /dev/mmcblk0p2 /tmp/mountedmmcblk0p2")
	if (false == ret) then
		return ret, out
	end

	ret, out = cmdexec("sed -i -e 's/main/alt/' /tmp/mountedmmcblk0p2/lib/systemd/system/swupdate.service")
	if (false == ret) then
		return ret, out
	end

	ret, out = cmdexec("sed -i -e 's/-c [0-3]/-c 2/' /tmp/mountedmmcblk0p2/lib/systemd/system/swupdate.service")
	if (false == ret) then
		return ret, out
	end

	ret, out = cmdexec("umount /tmp/mountedmmcblk0p2")
	if (false == ret) then
		return ret, out
	end

	ret, out = cmdexec("rm -rf /tmp/mountedmmcblk0p2")
	if (false == ret) then
		return ret, out
	end
